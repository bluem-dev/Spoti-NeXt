...
function itemMetaLine(item: BatchAnalysisItem): string {
    ...
    switch (item.status) {
        case "analyzing":
            return t("analyzingAudioQuality");
        case "error":
            return item.error || t("analysisFailed");
        case "pending":
        default:
            return t("analysisWaiting");
    }
}
...
export function AudioAnalysisPage({ onBack }: AudioAnalysisPageProps) {
    ...
    const runBatchAnalysis = useCallback(async (entries: BatchAnalysisItem[]) => {
        ...
        try {
                ...
                if (outcome.result) {
                    ...
                    setItems((prev) => prev.map((item) => item.id === entry.id
                        ? {
                            ...item,
                            status: "error",
                            error: outcome.error || t("analysisFailed"),
                        }
                        : item));
            ...
            if (batchRunIdRef.current === runId) {
                ...
                if (successCount > 0) {
                    toast.success(t("analysisBatchComplete"), {
                        description: t("analysisBatchCompleteDesc")
                            .replace("{success}", String(successCount))
                            .replace("{failed}", failCount > 0 ? `, ${failCount} ${t("analysisBatchFailedDesc").split(" ")[1]}` : ""),
                    });
                }
                else if (failCount > 0) {
                    toast.error(t("analysisBatchFailed"), {
                        description: t("analysisBatchFailedDesc")
    ...
    const ensureIdleQueue = useCallback(() => {
        ...
        toast.info(t("analysisInProgress"), {
            description: t("analysisInProgressDesc"),
        });
        return false;
    }, [isBatchRunning]);
    const addPathItems = useCallback(async (paths: string[]) => {
        ...
        if (invalidCount > 0) {
            toast.error(t("analysisUnsupportedFormat"), {
                description: t("analysisUnsupportedFormatDesc").replace("{label}", SUPPORTED_AUDIO_LABEL),
            });
        }
        ...
        if (newPaths.length === 0) {
            toast.info(t("analysisNoNewFiles"), {
                description: t("analysisNoNewFilesDesc"),
            });
            return;
        }
        ...
        if (validPaths.length !== newPaths.length) {
            toast.info(t("analysisSomeSkipped"), {
                description: t("analysisSomeSkippedDesc").replace("{count}", String(validPaths.length - newPaths.length)),
            });
        }
    ...
    const addBrowserFiles = useCallback(async (files: File[]) => {
        ...
        if (validFiles.length !== newItems.length) {
            toast.info(t("analysisSomeSkipped"), {
                description: t("analysisSomeSkippedDesc").replace("{count}", String(validFiles.length - newItems.length)),
            });
        }
        setItems((prev) => [...prev, ...newItems]);
        if (!activeItemIdRef.current) {
    ...
    const handleSelectFolder = useCallback(async () => {
        ...
        try {
            ...
            if (!folderFiles || folderFiles.length === 0) {
                toast.info(t("analysisNoAudioFiles"), {
                    description: t("analysisNoAudioFilesDesc").replace("{label}", SUPPORTED_AUDIO_LABEL),
                });
                return;
            }
            ...
            toast.error(t("analysisFolderFailed"), {
                description: err instanceof Error ? err.message : t("errorSelectingFolder"),
            });
        }
    }, [addPathItems, ensureIdleQueue]);
    ...
    const handleStopBatch = useCallback(() => {
        ...
        setItems((prev) => prev.map((item) => item.status === "analyzing"
            ...
            : item));
        toast.info(t("analysisBatchStopped"), {
            description: t("analysisBatchStoppedDesc"),
        });
    ...
    const handleExportSelected = useCallback(async () => {
        ...
        if (!dataUrl) {
            toast.error(t("analysisExportFailed"), {
                description: t("analysisExportCanvasError"),
            });
            return;
        }
        ...
        try {
            if (activeItem.source === "path" && isAbsolutePath(activeItem.path)) {
                ...
                toast.success(t("analysisPngExported"), {
                    description: t("analysisPngExportedSaved").replace("{path}", outPath),
                });
                return;
            }
            ...
            toast.success(t("analysisPngExported"), {
                description: t("analysisPngExportedDownloaded"),
            });
        }
        catch (err) {
            toast.error(t("analysisExportFailed"), {
                description: err instanceof Error ? err.message : t("analysisExportFailedDesc"),
            });
        }
        finally {
    ...
    const handleBatchExport = useCallback(async () => {
        ...
        if (exportableItems.length === 0) {
            toast.error(t("analysisNothingToExport"), {
                description: t("analysisNothingToExportDesc"),
            });
            return;
        }
        ...
        try {
            ...
            if (successCount > 0) {
                toast.success(t("analysisBatchPngComplete"), {
                    description: t("analysisBatchPngCompleteDesc")
                        .replace("{success}", String(successCount))
                        .replace("{failed}", failCount > 0 ? `, ${failCount} ${t("analysisBatchFailed").toLowerCase()}` : ""),
                });
            }
            else {
                toast.error(t("analysisBatchPngFailed"), {
                    description: t("analysisBatchPngFailedDesc"),
                });
            }
        }
        finally {
            ...
            <CardContent className="space-y-4">
                {activeItem.status === "analyzing" && (<div className="space-y-3">
                        <div className="flex items-center gap-3">
                            <Spinner />
                            <span className="text-sm text-muted-foreground">{t("analysisAnalyzingStatus")}</span>
                        </div>
                ...
                {activeItem.status === "error" && (<div className="rounded-lg border border-destructive/30 bg-destructive/5 p-4 text-sm text-destructive">
                        {activeItem.error || t("analysisFailed")}
                    </div>)}
            </CardContent>
    ...
    const singleModeContent = !activeItem ? null : activeItem.status === "success" && activeItem.result ? (<div className="mx-auto w-full max-w-6xl space-y-4">
            ...
            <div className="w-full max-w-md space-y-2">
                <div className="flex items-center justify-between text-sm text-muted-foreground">
                    <span>{activeItem.status === "pending" ? t("analysisPreparing") : t("analysisProcessing")}</span>
    ...
    return (<div className="space-y-6">
            ...
            <div className="flex flex-wrap items-center justify-between gap-3">
                <div className="flex items-center gap-4">
                    ...
                    <h1 className="text-2xl font-bold">{t("audioQualityAnalyzerTitle")}</h1>
                </div>
                ...
                <div className="flex flex-wrap gap-2">
                    ...
                    {showSingleModeActions && (<Button onClick={handleExportSelected} variant="outline" size="sm" disabled={isExportingSelected || spectrumLoading}>
                            ...
                            {isExportingSelected ? t("analysisExporting") : t("analysisExportPng")}
                        </Button>)}
                    {isBatchMode && (<DropdownMenu>
                            <DropdownMenuTrigger asChild>
                                <Button variant="outline" size="sm" disabled={successItems.length === 0 || isExportingBatch || isExportingSelected || isBatchRunning || spectrumLoading}>
                                    <Download className="h-4 w-4 mr-1"/>
                                    {isExportingBatch ? t("analysisExporting") : isExportingSelected ? t("analysisExporting") : t("analysisExport")}
                                    <ChevronDown className="ml-1 h-4 w-4"/>
                                </Button>
                            </DropdownMenuTrigger>
                ...
                : t("analysisDragDrop")}
                    ...
                    <div className="flex gap-3">
                        <Button onClick={handleSelectFiles} size="lg">
                            ...
                            {t("analysisSelectFiles")}
                        </Button>
                        <Button onClick={handleSelectFolder} size="lg" variant="outline">
                            <Upload className="h-5 w-5"/>
                            {t("analysisSelectFolder")}
                        </Button>
                    </div>
                    <p className="text-xs text-muted-foreground mt-4 text-center">
                        {t("analysisSupportedFormats")}
                    </p>
                </div>)}
                        ...
                        {(isBatchRunning || isExportingBatch) && (<Card className="gap-2 py-4">
                                <CardHeader className="px-4 pb-0">
                                    <CardTitle className="text-sm">
                                        {isExportingBatch ? t("analysisBatchPngExport") : t("analysisBatchAnalysis")}
                                    </CardTitle>
                                </CardHeader>
                                <CardContent className="space-y-2 px-4">
                                    <div className="flex items-center justify-between text-xs text-muted-foreground">
                                        <span className="truncate pr-3">
                                            {isExportingBatch
                    ? exportProgress.fileName || t("analysisPreparingExport")
                        ...
                        <Card className="gap-2 overflow-hidden py-4">
                            <CardHeader className="px-4 pb-0">
                                <div className="flex items-center justify-between gap-3">
                                    <CardTitle className="text-sm">{t("analysisBatchQueue")}</CardTitle>
                                    <p className="text-xs text-muted-foreground">
                                        {items.length} {t("statusQueued").replace(":", "")} • {successItems.length} ready
                                    </p>
                                </div>
                            </CardHeader>
