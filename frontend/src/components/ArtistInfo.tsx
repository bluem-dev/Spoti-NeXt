...
export function ArtistInfo({ artistInfo, albumList, trackList, searchQuery, sortBy, selectedTracks, downloadedTracks, failedTracks, skippedTracks, downloadingTrack, isDownloading, bulkDownloadType, downloadProgress, currentDownloadInfo, currentPage, itemsPerPage, downloadedLyrics, failedLyrics, skippedLyrics, downloadingLyricsTrack, checkingAvailabilityTrack, availabilityMap, downloadedCovers, failedCovers, skippedCovers, downloadingCoverTrack, isBulkDownloadingCovers, isBulkDownloadingLyrics, onSearchChange, onSortChange, onToggleTrack, onToggleSelectAll, onDownloadTrack, onDownloadLyrics, onDownloadCover, onCheckAvailability, onDownloadAllLyrics, onDownloadAllCovers, onDownloadAll, onDownloadSelected, onStopDownload, onOpenFolder, onAlbumClick, onArtistClick, onPageChange, onTrackClick, onBack, }: ArtistInfoProps) {
        ...
        try {
            ...
            if (response.success) {
                if (response.already_exists) {
                    toast.info(t("headerAlreadyExists"));
                }
                else {
                    toast.success(t("headerDownloadedSuccessfully"));
                }
            }
            else {
                toast.error(response.error || t("failedToDownloadHeader"));
            }
        }
        catch (error) {
            toast.error(t("errorDownloadingHeader"));
        }
        finally {
    ...
    const handleDownloadAvatar = async () => {
        ...
        try {
            ...
            if (response.success) {
                if (response.already_exists) {
                    toast.info(t("avatarAlreadyExists"));
                }
                else {
                    toast.success(t("avatarDownloadedSuccessfully"));
                }
            }
            else {
                toast.error(response.error || t("failedToDownloadAvatar"));
            }
        }
        catch (error) {
            toast.error(t("errorDownloadingAvatar"));
        }
        finally {
    ...
    const handleDownloadGalleryImage = async (imageUrl: string, index: number) => {
        ...
        try {
            ...
            if (response.success) {
                if (response.already_exists) {
                    toast.info(t("galleryImageAlreadyExists").replace("{n}", String(index + 1)));
                }
                else {
                    toast.success(t("galleryImageDownloaded").replace("{n}", String(index + 1)));
                }
            }
            else {
                toast.error(response.error || t("failedToDownloadGalleryImage").replace("{n}", String(index + 1)));
            }
        }
        catch (error) {
            toast.error(t("failedToDownloadGalleryImage").replace("{n}", String(index + 1)));
        }
        finally {
        ...
        try {
            ...
            if (failCount === 0) {
                if (existsCount > 0 && successCount > 0) {
                    toast.success(t("galleryImagesDownloaded").replace("{s}", String(successCount)).replace("{e}", String(existsCount)));
                }
                else if (existsCount > 0) {
                    toast.info(t("allGalleryImagesExist").replace("{n}", String(existsCount)));
                }
                else {
                    toast.success(t("allGalleryImagesDownloaded").replace("{n}", String(successCount)));
                }
            }
            else {
                toast.error(t("galleryImagesFailed").replace("{n}", String(failCount)));
            }
        }
        catch (error) {
            toast.error(t("errorDownloadingGallery"));
        }
        finally {
                ...
                <div className="flex gap-6 items-start">
                  ...
                  <div className="flex-1 space-y-2">
                    ...
                    <div className="flex items-center gap-2 text-sm flex-wrap text-white/90">
                      ...
                      <span>{artistInfo.followers.toLocaleString()} {artistInfo.followers === 1 ? t("follower") : t("followers")}</span>
                      {artistInfo.listeners && (<>
                          <span>•</span>
                          <span>{artistInfo.listeners.toLocaleString()} {artistInfo.listeners === 1 ? t("listener") : t("listeners")}</span>
                        </>)}
                    </div>
                    <div className="flex items-center gap-2 text-sm flex-wrap text-white/90">
            ...
            <div className="flex gap-2 flex-wrap">
              <Dialog>
                  ...
                  <DialogContent className="sm:max-w-[500px] h-[80vh] flex flex-col">
                      <DialogHeader>
                          <DialogTitle>{t("selectAlbums")}</DialogTitle>
                      </DialogHeader>
