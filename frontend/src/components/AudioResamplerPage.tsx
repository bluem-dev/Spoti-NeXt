...
const STORAGE_KEY = "spotinext_audio_resampler_state";
export function AudioResamplerPage() {
    ...
    const handleSelectFiles = async () => {
        try {
            ...
            toast.error(t("resamplerFileSelectionFailed"), {
                description: err instanceof Error ? err.message : t("resamplerFailedSelectFiles"),
            });
        }
    };
        ...
        try {
            ...
            if (selectedFolder) {
                ...
                if (folderFiles && folderFiles.length > 0) {
                    ...
                    toast.info(t("resamplerNoAudioFilesFound"), {
                        description: t("resamplerNoFlacFiles"),
                    });
                }
            }
        }
        catch (err) {
            toast.error(t("resamplerFolderSelectionFailed"), {
                description: err instanceof Error ? err.message : t("resamplerFailedSelectFolder"),
            });
        }
    };
    const addFiles = useCallback(async (paths: string[]) => {
        ...
        if (invalidFiles.length > 0) {
            toast.error(t("resamplerUnsupportedFormat"), {
                description: t("resamplerOnlyFlac"),
            });
        }
        ...
        setFiles((prev) => {
            ...
            if (newFiles.length > 0) {
                if (paths.length > newFiles.length + invalidFiles.length) {
                    ...
                    toast.info(t("resamplerSomeFilesSkipped"), {
            ...
            if (validPaths.length > 0 && newFiles.length === 0) {
                toast.info(t("resamplerNoNewFilesAdded"), {
                    description: t("resamplerAllAdded"),
                });
            }
            return prev;
        });
    ...
    const handleResample = async () => {
        if (files.length === 0) {
            toast.error(t("resamplerNoFilesSelected"), {
                description: t("resamplerPleaseAdd"),
            });
            return;
        }
        setResampling(true);
    ...
    return (<div className={`space-y-6 ${isFullscreen ? "h-full flex flex-col" : ""}`}>
        ...
        <div className="flex items-center justify-between">
            <h1 className="text-2xl font-bold">{t("audioResamplerTitle")}</h1>
