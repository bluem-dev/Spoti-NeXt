...
export function Sidebar({ currentPage, onPageChange }: SidebarProps) {
    ...
    const handleOpenIssues = () => {
        openExternal("https://github.com/afkarxyz/Spoti NeXt/issues");
                ...
                <DropdownMenu>
                    ...
                    <DropdownMenuContent side="right" sideOffset={14} className="min-w-[200px] ml-2">
                        <DropdownMenuItem onClick={() => onPageChange("audio-analysis")} className="gap-3 cursor-pointer py-2 px-3" {...getAnimatedItemHandlers(analyzerIconRef)}>
                            ...
                            <span>{t("audioQualityAnalyzer")}</span>
                        </DropdownMenuItem>
            ...
            <div className="mt-auto flex flex-col gap-2">
                <Dialog open={isIssuesDialogOpen} onOpenChange={handleIssuesDialogChange}>
                    <Tooltip delayDuration={0}>
                        ...
                        <TooltipContent side="right">
                            <p>{t("reportBugsOrRequestFeatures")}</p>
                        </TooltipContent>
                    </Tooltip>
                    <DialogContent className="max-w-xl">
                        <DialogHeader>
                            <DialogTitle>{t("beforeOpeningGitHubIssues")}</DialogTitle>
                            <DialogDescription />
                        </DialogHeader>

                        <div className="space-y-4 text-sm">
                            <div className="rounded-lg border border-amber-500/40 bg-amber-500/10 p-4">
                                <p className="font-semibold text-amber-900 dark:text-amber-200">{t("issuesDialogImportant")}</p>
                                <p className="mt-1 text-amber-950/90 dark:text-amber-100/90">
                                    {t("issuesDialogWarning")}
                                </p>
                            </div>
