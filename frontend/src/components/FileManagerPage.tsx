...
const STORAGE_KEY = "spotinext_file_manager_state";
...
export function FileManagerPage() {
    ...
    const loadFiles = useCallback(async () => {
        ...
        try {
            ...
            if (!errorMsg.toLowerCase().includes("empty") && !errorMsg.toLowerCase().includes("no file")) {
                toast.error(t("failedToLoadFiles"), { description: errorMsg || "Unknown error" });
            }
            setAllFiles([]);
            setSelectedFiles(new Set());
        }
    ...
    const handleSelectFolder = async () => {
        try {
            ...
            toast.error(t("failedToSelectFolder"), { description: err instanceof Error ? err.message : "Unknown error" });
        }
    };
    ...
    const handlePreview = async (isPreviewOnly: boolean) => {
        ...
        try {
            ...
            toast.error(t("failedToGeneratePreview"), { description: err instanceof Error ? err.message : "Unknown error" });
        }
    };
    const handleShowMetadata = async (filePath: string, e: React.MouseEvent) => {
        ...
        try {
            ...
            toast.error(t("failedToReadMetadata"), { description: err instanceof Error ? err.message : "Unknown error" });
    ...
    const handleShowLyrics = async (filePath: string, e: React.MouseEvent) => {
        ...
        try {
            ...
            toast.error(t("failedToReadLyrics"), { description: err instanceof Error ? err.message : "Unknown error" });
        }
    };
    const handleShowCover = async (filePath: string, e: React.MouseEvent) => {
        ...
        try {
            ...
            toast.error(t("failedToLoadImage"), { description: err instanceof Error ? err.message : "Unknown error" });
        }
    };
    ...
    const handleCopyLyrics = async () => {
        try {
            ...
            toast.error(t("failedToCopyLyrics"));
        }
    };
    ...
    const handleConfirmManualRename = async () => {
        ...
        try {
            ...
            toast.error(t("failedToRenameFile"), { description: err instanceof Error ? err.message : "Unknown error" });
        }
        finally {
    ...
    const handleRename = async () => {
        ...
        try {
            ...
            if (successCount > 0)
                toast.success(t("renameComplete"), { description: `${successCount} file(s) renamed${failCount > 0 ? `, ${failCount} failed` : ""}` });
            else
                toast.error(t("renameFailed"), { description: `All ${failCount} file(s) failed to rename` });
            setShowPreview(false);
            setSelectedFiles(new Set());
            loadFiles();
        }
        catch (err) {
            toast.error(t("renameFailed"), { description: err instanceof Error ? err.message : "Unknown error" });
        }
        finally {
            setRenaming(false);
        }
      ...
      <div className={`flex items-center gap-2 py-1.5 px-2 rounded hover:bg-muted/50 cursor-pointer ${selectedFiles.has(node.path) ? "bg-primary/10" : ""}`} style={{ paddingLeft: `${depth * 16 + 8}px` }} onClick={() => (node.is_dir ? toggleExpand(node.path) : toggleSelect(node.path))}>
        ...
        {!node.is_dir && (<>
          ...
          <Tooltip>
            ...
            <TooltipContent>{t("viewMetadata")}</TooltipContent>
          </Tooltip>
        </>)}
      </div>
    ...
    <div className="flex items-center justify-between shrink-0">
      <h1 className="text-2xl font-bold">{t("fileManagerTitle")}</h1>
    </div>
    ...
    <div className={`border rounded-lg ${isFullscreen ? "flex-1 flex flex-col min-h-0" : ""}`}>
      ...
      <div className={`overflow-y-auto p-2 ${isFullscreen ? "flex-1 min-h-0" : "max-h-[400px]"}`}>
        {loading ? (<div className="flex items-center justify-center py-8"><Spinner className="h-6 w-6"/></div>) : filteredFiles.length === 0 ? (<div className="text-center py-8 text-muted-foreground">
          {rootPath ? `No ${activeTab} files found` : t("selectFolderToBrowse")}
    ...
    <Dialog open={showPreview} onOpenChange={setShowPreview}>
      <DialogContent className="max-w-2xl max-h-[80vh] overflow-hidden flex flex-col [&>button]:hidden">
        <DialogHeader>
          <DialogTitle>{t("renamePreview")}</DialogTitle>
          <DialogDescription>{t("renamePreviewDesc")}</DialogDescription>
        </DialogHeader>
        ...
        <DialogFooter>
          {previewOnly ? (<Button onClick={() => setShowPreview(false)}>{t("close")}</Button>) : (<>
            <Button variant="outline" onClick={() => setShowPreview(false)}>{t("cancel")}</Button>
            <Button onClick={handleRename} disabled={renaming}>
              {renaming ? <><Spinner className="h-4 w-4"/>Renaming...</> : <>{t("rename")} {previewData.filter((p) => !p.error).length} File(s)</>}
            </Button>
          </>)}
        </DialogFooter>
      </DialogContent>
    ...
    <Dialog open={showMetadata} onOpenChange={setShowMetadata}>
      <DialogContent className="max-w-md [&>button]:hidden">
        <DialogHeader>
          <DialogTitle>{t("fileMetadata")}</DialogTitle>
        ...
        <DialogFooter><Button onClick={() => setShowMetadata(false)}>{t("close")}</Button></DialogFooter>
      </DialogContent>
    </Dialog>
    ...
    <Dialog open={showLyricsPreview} onOpenChange={setShowLyricsPreview}>
      <DialogContent className="max-w-lg max-h-[80vh] overflow-hidden flex flex-col [&>button]:hidden">
        <DialogHeader>
          <DialogTitle>{t("tabLyric")}</DialogTitle>
          <DialogDescription className="break-all">{lyricsFile.split(/[/\\]/).pop()}</DialogDescription>
        </DialogHeader>
        <div className="flex gap-2 border-b pb-2">
          <Button variant={lyricsTab === "synced" ? "default" : "ghost"} size="sm" onClick={() => setLyricsTab("synced")}>{t("synced")}</Button>
          <Button variant={lyricsTab === "plain" ? "default" : "ghost"} size="sm" onClick={() => setLyricsTab("plain")}>{t("plain")}</Button>
        </div>
        <div className="flex-1 overflow-y-auto py-4">
          {lyricsTab === "synced" ? (<div className="bg-muted/30 p-4 rounded-lg space-y-0">
            ...
            {getPlainLyrics(lyricsContent) || t("noLyricsContent")}
          </pre>)}
        </div>
        <DialogFooter>
          ...
          <Button onClick={() => setShowLyricsPreview(false)}>{t("close")}</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
    ...
    <Dialog open={showCoverPreview} onOpenChange={setShowCoverPreview}>
      <DialogContent className="max-w-lg [&>button]:hidden">
        <DialogHeader>
          <DialogTitle>{t("coverPreview")}</DialogTitle>
          <DialogDescription className="break-all">{coverFile.split(/[/\\]/).pop()}</DialogDescription>
        </DialogHeader>
        <div className="flex items-center justify-center p-4">
          {coverData ? <img src={coverData} alt={t("tabCover")} className="max-w-full max-h-[350px] rounded-lg object-contain"/> : <div className="text-muted-foreground">Loading...</div>}
        </div>
        <DialogFooter><Button onClick={() => setShowCoverPreview(false)}>{t("close")}</Button></DialogFooter>
      </DialogContent>
    </Dialog>
    ...
    <Dialog open={showManualRename} onOpenChange={setShowManualRename}>
      <DialogContent className="max-w-2xl [&>button]:hidden">
        ...
        <div className="py-4">
          <Label htmlFor="newName" className="text-sm">{t("newName")}</Label>
          <div className="flex items-center gap-2 mt-2">
            <InputWithContext id="newName" value={manualRenameName} onChange={(e) => setManualRenameName(e.target.value)} placeholder={t("newName")} className="flex-1" onKeyDown={(e) => {
        ...
        <DialogFooter>
          <Button variant="outline" onClick={() => setShowManualRename(false)} disabled={manualRenaming}>{t("cancel")}</Button>
          <Button onClick={handleConfirmManualRename} disabled={manualRenaming || !manualRenameName.trim()}>
            {manualRenaming ? <><Spinner className="h-4 w-4"/>Renaming...</> : t("rename")}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
