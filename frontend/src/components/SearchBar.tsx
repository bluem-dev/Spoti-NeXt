...
import { SearchQobuz } from "@/lib/wails-extra";
...
const FETCH_PLACEHOLDERS_BY_SOURCE: Record<string, string[]> = {
    spotify: [
...
const FETCH_PLACEHOLDERS = FETCH_PLACEHOLDERS_BY_SOURCE.spotify;
...
const RECENT_SEARCHES_KEY = "spotinext_recent_searches";
...
export function SearchBar({ url, loading, onUrlChange, onFetch, onFetchUrl, history, onHistorySelect, onHistoryRemove, hasResult, searchMode, onSearchModeChange, region, onRegionChange, }: SearchBarProps) {
    ...
    const { directMode, downloader } = getSettings();
    const isDirectMode = directMode && downloader !== "auto";
    const activeFetchPlaceholders = isDirectMode && downloader in FETCH_PLACEHOLDERS_BY_SOURCE
        ? FETCH_PLACEHOLDERS_BY_SOURCE[downloader]
        : FETCH_PLACEHOLDERS;
    const placeholders = searchMode ? SEARCH_PLACEHOLDERS : activeFetchPlaceholders;
    ...
    useEffect(() => {
        ...
        searchTimeoutRef.current = setTimeout(async () => {
            setIsSearching(true);
            try {
                if (isDirectMode && downloader === "qobuz") {
                    const tracks = await SearchQobuz({
                        query: searchQuery,
                        limit: SEARCH_LIMIT,
                    });
                    const results = { tracks: tracks ?? [], albums: [], artists: [], playlists: [] };
                    setSearchResults(results as any);
                    setResultFilter("");
                    setLastSearchedQuery(searchQuery.trim());
                    saveRecentSearch(searchQuery.trim());
                    setHasMore({ tracks: false, albums: false, artists: false, playlists: false });
    ...
    const DIRECT_SOURCE_DOMAINS: Record<string, string[]> = {
        tidal:  ["tidal.com"],
        qobuz:  ["qobuz.com"],
        amazon: ["music.amazon.com", "amazon.com/music"],
    };
    const isValidFetchUrl = (text: string) => {
        ...
        // Always allow Spotify URLs
        if (trimmed.includes("spotify.com") || trimmed.includes("spotify.link") || trimmed.startsWith("spotify:"))
            return true;
        // In direct mode allow the active source URLs too
        if (isDirectMode && downloader in DIRECT_SOURCE_DOMAINS) {
            return DIRECT_SOURCE_DOMAINS[downloader].some(d => trimmed.includes(d));
        }
        return false;
    };
    const handlePaste = (e: React.ClipboardEvent<HTMLInputElement>) => {
        ...
        if (pastedText && !isValidFetchUrl(pastedText)) {
            e.preventDefault();
    ...
    const tabs: {
        ...
        { key: "tracks", label: t("tabTracks") },
        { key: "albums", label: t("tabAlbums") },
        { key: "artists", label: t("tabArtists") },
        { key: "playlists", label: t("tabPlaylists") },
    ];
    return (<div className="space-y-4">
      <div className="flex gap-2">
        <Tooltip>
          ...
          <TooltipContent>
            <p>{searchMode ? t("fetchMode") : t("searchMode")}</p>
          </TooltipContent>
        </Tooltip>
      ...
      {searchMode && (<div className="space-y-4">
          {!searchQuery && !searchResults && recentSearches.length > 0 && (<div className="space-y-2">
              <p className="text-sm text-muted-foreground">{t("recentSearches")}</p>
          ...
          {!isSearching && hasAnyResults && (<>
              ...
              <div className="flex gap-2 mb-4">
                <div className="relative flex-1">
                  ...
                  <Input placeholder={`${t("searchTracks").replace("tracks...", `${activeTab}...`)}`} value={resultFilter} onChange={(e) => setResultFilter(e.target.value)} className="pl-10 pr-8"/>
                  ...
                  <SelectTrigger className="w-[170px] bg-background gap-1.5">
                    ...
                    <SelectContent>
                      ...
                      {activeTab === 'albums' && (<>
                          ...
                          <SelectItem value="artist-asc">{t("sortArtistAZ")}</SelectItem>
                          <SelectItem value="artist-desc">{t("sortArtistZA")}</SelectItem>
                          <SelectItem value="year-desc">{t("sortDateNewest")}</SelectItem>
                          <SelectItem value="year-asc">{t("sortDateOldest")}</SelectItem>
                        </>)}
                      {activeTab === 'artists' && (<>
                          <SelectItem value="name-asc">{t("sortTitleAZ")}</SelectItem>
                          <SelectItem value="name-desc">{t("sortTitleZA")}</SelectItem>
                        </>)}
                      {activeTab === 'playlists' && (<>
                          <SelectItem value="title-asc">{t("sortTitleAZ")}</SelectItem>
                          <SelectItem value="title-desc">{t("sortTitleZA")}</SelectItem>
                          <SelectItem value="owner-asc">{t("sortArtistAZ")}</SelectItem>
                          <SelectItem value="owner-desc">{t("sortArtistZA")}</SelectItem>
                        </>)}
                    </SelectContent>
                  </Select>
              </div>
      ...
      <Dialog open={showInvalidUrlDialog} onOpenChange={setShowInvalidUrlDialog}>
        <DialogContent className="sm:max-w-[425px]">
          <DialogHeader>
            <DialogTitle>{t("invalidUrl")}</DialogTitle>
            <DialogDescription>
              {isDirectMode
                ? t("invalidUrlDesc").replace("Only Spotify links are allowed", `Only Spotify or ${SOURCE_LABELS[downloader] ?? downloader} links are allowed`)
                : t("invalidUrlDesc")}
            </DialogDescription>
          </DialogHeader>
