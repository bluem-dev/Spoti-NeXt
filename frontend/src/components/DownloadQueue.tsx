...
export function DownloadQueue({ isOpen, onClose }: DownloadQueueProps) {
    ...
    const handleReset = async () => {
        try {
            ...
            const info = await GetDownloadQueue();
            setQueueInfo(info);
            toast.success(t("downloadQueueReset"));
        }
        catch (error) {
    ...
    <DialogContent className="max-w-[1200px] w-[95vw] max-h-[80vh] flex flex-col p-0 gap-0 [&>button]:hidden">
      <DialogHeader className="px-6 pt-6 pb-4 border-b space-y-0">
        <div className="flex items-center justify-between mb-4">
          <DialogTitle className="text-lg font-semibold hover:text-primary transition-colors cursor-pointer" onClick={handleReset}>{t("downloadQueue")}</DialogTitle>
        ...
        <div className="space-y-2 py-4">
          {queueInfo.queue.length === 0 ? (<div className="text-center py-12 text-muted-foreground">
            ...
            <p>{t("noDownloadsInQueue")}</p>
             ...
             <Button variant="link" onClick={() => setFilterStatus("all")}>{t("clearFilter")}</Button>
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                {item.status === t("statusDownloading") && (<div className="flex items-center gap-3 mt-1.5 text-xs text-muted-foreground font-mono">
                  <span>
                    {item.progress > 0
                ...
                {item.status === t("statusCompleted") && (<div className="flex items-center gap-3 mt-1.5 text-xs text-muted-foreground">
                  <span className="font-mono">{item.progress.toFixed(2)} MB</span>
                </div>)}


                {item.status === t("statusSkipped") && (<div className="mt-1.5 text-xs text-muted-foreground">
                  {t("fileAlreadyExistsMsg")}
                </div>)}


                {item.status === t("statusFailed") && item.error_message && (<div className="mt-1.5 text-xs text-red-500 bg-red-50 dark:bg-red-950/20 rounded px-2 py-1">
                  {item.error_message}
                </div>)}


                {(item.status === t("statusCompleted") || item.status === t("statusSkipped")) && item.file_path && (<div className="mt-1.5 text-xs text-muted-foreground truncate font-mono">
                  {item.file_path}
                </div>)}
              </div>
            </div>
