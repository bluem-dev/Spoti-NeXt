...
const STORAGE_KEY = "spotinext_audio_converter_state";
export function AudioConverterPage() {
    ...
    const handleSelectFiles = async () => {
        try {
            ...
            toast.error(t("fileSelectionFailed"), {
        ...
        try {
            ...
            if (selectedFolder) {
                ...
                if (folderFiles && folderFiles.length > 0) {
                    ...
                    toast.info(t("noAudioFilesFound"), {
            ...
            toast.error(t("folderSelectionFailed"), {
    ...
    const addFiles = useCallback(async (paths: string[]) => {
        ...
        setFiles((prev) => {
            ...
            if (paths.length > 0 && m4aFiles.length === 0) {
                toast.info(t("noNewFilesAdded"), {
                    description: t("allFilesAlreadyAdded"),
                });
            }
            return prev;
        });
    ...
    const handleConvert = async () => {
        if (files.length === 0) {
            toast.error(t("noFilesSelected"), {
                description: t("pleaseAddAudioFiles"),
            });
            return;
        }
        setConverting(true);
    ...
    return (<div className={`space-y-6 ${isFullscreen ? "h-full flex flex-col" : ""}`}>
        ...
        <div className="flex items-center justify-between">
            <h1 className="text-2xl font-bold">{t("audioConverterTitle")}</h1>
