import { CheckAPIStatus, CheckAtmosSupport } from "../../wailsjs/go/main/App";
...
type ApiStatusState = {
    ...
    // audioMode returned by each Tidal API for a DOLBY_ATMOS probe request.
    // Only populated for type === "tidal". Values: "DOLBY_ATMOS" | "STEREO" | "UNKNOWN" | "checking" | ""
    atmosModes: Record<string, string>;
};
...
async function checkSingleApiStatus(source: ApiSource): Promise<void> {
    setApiStatusState((current) => ({
        ...current,
        statuses: { ...current.statuses, [source.id]: "checking" },
        atmosModes: source.type === "tidal"
            ? { ...current.atmosModes, [source.id]: "checking" }
            : current.atmosModes,
    }));
    try {
        ...
        setApiStatusState((current) => ({
            ...current,
            statuses: { ...current.statuses, [source.id]: isOnline ? "online" : "offline" },
        }));

        // For Tidal APIs that are online, probe Atmos support concurrently.
        if (source.type === "tidal" && isOnline) {
            CheckAtmosSupport(source.url)
                .then((audioMode) => {
                    setApiStatusState((current) => ({
                        ...current,
                        atmosModes: { ...current.atmosModes, [source.id]: audioMode },
                    }));
                })
                .catch(() => {
                    setApiStatusState((current) => ({
                        ...current,
                        atmosModes: { ...current.atmosModes, [source.id]: "UNKNOWN" },
                    }));
                });
        } else if (source.type === "tidal" && !isOnline) {
            setApiStatusState((current) => ({
                ...current,
                atmosModes: { ...current.atmosModes, [source.id]: "" },
            }));
        }
    }
    catch {
        setApiStatusState((current) => ({
            ...current,
            statuses: { ...current.statuses, [source.id]: "offline" },
            atmosModes: source.type === "tidal"
                ? { ...current.atmosModes, [source.id]: "" }
                : current.atmosModes,
        }));
    }
}
