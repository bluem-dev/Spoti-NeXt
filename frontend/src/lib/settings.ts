...
export interface Settings {
    ...
    language: "en" | "es";
    directMode: boolean;
    // Tidal account (device-link OAuth) — debug/experimental
    tidalAccountAccessToken?: string;
    tidalAccountRefreshToken?: string;
    tidalAccountExpiresAt?: string;
    tidalAccountCountryCode?: string;
    tidalAccountUserID?: number;
    tidalAccountClientID?: string;
}
...
export const DEFAULT_SETTINGS: Settings = {
    ...
    language: "en",
    directMode: false,
    tidalAccountAccessToken: "",
    tidalAccountRefreshToken: "",
    tidalAccountExpiresAt: "",
    tidalAccountCountryCode: "",
    tidalAccountUserID: 0,
    tidalAccountClientID: ""
};
...
const SETTINGS_KEY = "spotinext-settings";
...
function getSettingsFromLocalStorage(): Settings {
    try {
        ...
        if (stored) {
            ...
            if (!('directMode' in parsed)) {
                parsed.directMode = false;
            }
            if (!('includeAtmos' in parsed)) {
                parsed.includeAtmos = false;
            }
            if (!('tidalAccountAccessToken' in parsed)) {
                parsed.tidalAccountAccessToken = "";
            }
            if (!('tidalAccountRefreshToken' in parsed)) {
                parsed.tidalAccountRefreshToken = "";
            }
            if (!('tidalAccountExpiresAt' in parsed)) {
                parsed.tidalAccountExpiresAt = "";
            }
            if (!('tidalAccountCountryCode' in parsed)) {
                parsed.tidalAccountCountryCode = "";
            }
            if (!('tidalAccountUserID' in parsed)) {
                parsed.tidalAccountUserID = 0;
            }
            if (!('tidalAccountClientID' in parsed)) {
                parsed.tidalAccountClientID = "";
            }
...
// Translated label helpers — call these at render time, not at import time,
// so the active language is applied correctly.
export function getFolderPresetLabel(preset: FolderPreset): string {
    const keyMap: Record<FolderPreset, import("@/lib/i18n").TranslationKey> = {
        "none": "folderNone",
        "artist": "folderArtist",
        "album": "folderAlbum",
        "year-album": "folderYearAlbum",
        "year-artist-album": "folderYearArtistAlbum",
        "artist-album": "folderArtistAlbum",
        "artist-year-album": "folderArtistYearAlbum",
        "artist-year-nested-album": "folderArtistYearNested",
        "album-artist": "folderAlbumArtist",
        "album-artist-album": "folderAlbumArtistAlbum",
        "album-artist-year-album": "folderAlbumArtistYearAlbum",
        "album-artist-year-nested-album": "folderAlbumArtistYearNested",
        "year": "folderYear",
        "year-artist": "folderYearArtist",
        "custom": "folderCustom",
    };
    return t(keyMap[preset] ?? preset);
}

export function getFilenamePresetLabel(preset: FilenamePreset): string {
    const keyMap: Record<FilenamePreset, import("@/lib/i18n").TranslationKey> = {
        "title": "fmtTitle",
        "title-artist": "fmtTitleArtist",
        "artist-title": "fmtArtistTitle",
        "track-title": "fmtTrackTitle",
        "track-title-artist": "fmtTrackTitleArtist",
        "track-artist-title": "fmtTrackArtistTitle",
        "title-album-artist": "fmtTitleAlbumArtist",
        "track-title-album-artist": "fmtTrackTitleAlbumArtist",
        "artist-album-title": "fmtArtistAlbumTitle",
        "track-dash-title": "fmtTrackDashTitle",
        "disc-track-title": "fmtDiscTrackTitle",
        "disc-track-title-artist": "fmtDiscTrackTitleArtist",
        "custom": "fmtCustom",
    };
    return t(keyMap[preset] ?? preset);
}
