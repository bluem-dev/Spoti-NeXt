import { useState, useEffect, useCallback, useLayoutEffect, useReducer } from "react";
...
import { OpenFolder, CheckFFmpegInstalled, DownloadFFmpeg, GetBrewPath, InstallFFmpegWithBrew, TidalAccountLoadSession } from "../wailsjs/go/main/App";
import { EventsOn, EventsOff, Quit } from "../wailsjs/runtime/runtime";
import { logger } from "@/lib/logger";
import { toastWithSound as toast } from "@/lib/toast-with-sound";
import { t, setLanguage, onLanguageChange, type Language } from "@/lib/i18n";
...
const HISTORY_KEY = "spotinext_fetch_history";
const MAX_HISTORY = 5;
...
function App() {
    ...
    const [region, setRegion] = useState(() => localStorage.getItem("spotinext_region") || "US");
    useEffect(() => {
        localStorage.setItem("spotinext_region", region);
    }, [region]);
    ...
    // Initialise language from persisted settings on first render
    useLayoutEffect(() => {
    ...
    // Re-render the entire tree when language changes
    const [, forceUpdate] = useReducer((x: number) => x + 1, 0);
    useEffect(() => onLanguageChange(forceUpdate), []);
    ...
    useEffect(() => {
        const initSettings = async () => {
            ...
            // Restore Tidal account session if credentials are persisted.
            if (settings.tidalAccountAccessToken) {
                TidalAccountLoadSession(
                    settings.tidalAccountAccessToken,
                    settings.tidalAccountRefreshToken ?? "",
                    settings.tidalAccountExpiresAt ?? "",
                    settings.tidalAccountCountryCode ?? "",
                    settings.tidalAccountClientID ?? "",
                    settings.tidalAccountUserID ?? 0
        ...
        EventsOn("debug:atmos", (msg: string) => {
            logger.debug(msg);
        });
        checkFFmpeg();
    ...
    const handleEnableSpotFetchApi = async () => {
        try {
            ...
            toast.success(t("spotFetchAPIEnabled"));
        }
        catch (err) {
...
EventsOn("ffmpeg:progress", (progress: number) => {
            ...
            if (response.success) {
                toast.success(useBrew ? t("ffmpegInstalledSuccessfully") : t("ffmpegInstalledSuccessfully"));
                setIsFFmpegInstalled(true);
            }
            else {
                toast.error(`${t("failedToInstallFFmpeg")}: ${response.error}`);
            }
        }
        catch (error) {
            console.error("Error installing FFmpeg:", error);
            toast.error(`${t("errorDuringFFmpegInstallation")}: ${error}`);
        }
        finally {
    ...
    const handleOpenFolder = async () => {
        ...
        if (!settings.downloadPath) {
            toast.error(t("downloadPathNotSet"));
            return;
        }
        try {
            ...
            toast.error(`${t("errorOpeningFolder")}: ${error}`);
        }
    };
                ...
                <DialogContent className="max-w-[450px] [&>button]:hidden p-6 gap-5">
                    <DialogHeader className="space-y-2">
                        ...
                        <DialogDescription className="text-sm text-foreground/70 leading-relaxed font-normal">
                            {brewPath ? (<>
                                    {t("ffmpegEssential")}
                                    {t("ffmpegEssentialBrew")} <span className="text-foreground font-semibold">brew install ffmpeg</span>
                                </>) : (<>
                                    {t("ffmpegEssential")}
                                    {t("ffmpegEssentialSize")} <span className="text-foreground font-semibold">100-200MB</span> {t("ffmpegDataSize")}
                                </>)}
                        </DialogDescription>
                    </DialogHeader>
                    ...
                    <DialogFooter className={`flex-row gap-3 pt-2 ${brewPath ? 'flex-col' : ''}`}>
                        ...
                        {brewPath ? (<Button className="flex-1 h-11 text-sm font-bold shadow-lg shadow-primary/10" onClick={() => handleInstallFFmpeg(true)} disabled={isInstallingFFmpeg}>
                                    {isInstallingFFmpeg ? "Installing..." : t("installViaBrew")}
                                ...
                                {isInstallingFFmpeg ? "Installing..." : t("installNow")}
                            </Button>)}
                    </DialogFooter>
                </DialogContent>
            </Dialog>
