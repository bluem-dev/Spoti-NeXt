...
export function useCover() {
        ...
        try {
            ...
            if (response.success) {
                if (response.already_exists) {
                    toast.info(t("coverFileAlreadyExists"));
                    setSkippedCovers((prev) => new Set(prev).add(id));
                }
                else {
                    toast.success(t("coverDownloadedSuccessfully"));
    ...
    const handleDownloadAllCovers = async (tracks: TrackMetadata[], playlistName?: string, isAlbum?: boolean) => {
        if (tracks.length === 0) {
            toast.error(t("noTracksForCovers"));
            return;
        }
        const settings = getSettings();
        ...
        for (let i = 0; i < tracks.length; i++) {
            if (stopBulkDownloadRef.current) {
                toast.info(t("coverDownloadStopped"));
                break;
            }
