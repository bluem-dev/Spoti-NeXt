...
export function useMetadata() {
        ...
        try {
            ...
            if ("playlist_info" in data) {
                ...
                if (!playlistInfo.owner.name && playlistInfo.tracks.total === 0 && data.track_list.length === 0) {
                    ...
                    toast.error(t("playlistNotFound"));
                    setMetadata(null);
                    return;
                }
            }
            ...
            toast.success(t("metadataFetchedSuccessfully"));
        }
        catch (err) {
            const errorMsg = err instanceof Error ? err.message : t("failedToFetchMetadata");
            logger.error(`fetch failed: ${errorMsg}`);
            const settings = getSettings();
            if (!settings.useSpotFetchAPI) {
                // Direct Spotify fetch failed — suggest enabling SpotFetch API.
                setShowApiModal(true);
            }
            else {
                // SpotFetch API is enabled but also failed — show error and
                // suggest the user retry (the backend already fell back to direct fetch).
                toast.error(errorMsg);
                logger.warning("SpotFetch API failed; direct Spotify fetch was attempted as fallback");
            }
        }
        finally {
            setLoading(false);
    ...
    const loadFromCache = (cachedData: string) => {
        try {
            ...
            setMetadata(data);
            toast.success(t("loadedFromCache"));
        }
        catch (err) {
            console.error("Failed to load from cache:", err);
            toast.error(t("failedToLoadFromCache"));
        }
    };
    const handleFetchMetadata = async (url: string) => {
        if (!url.trim()) {
            logger.warning("empty url provided");
            toast.error(t("pleaseEnterSpotifyUrl"));
            return;
        }
    ...
    const handleConfirmAlbumFetch = async () => {
        ...
        try {
            ...
            logger.info(`fetch completed in ${elapsed}s`);
            toast.success(t("albumMetadataFetchedSuccessfully"));
            return albumUrl;
        }
        catch (err) {
            const errorMsg = err instanceof Error ? err.message : t("failedToFetchMetadata");
