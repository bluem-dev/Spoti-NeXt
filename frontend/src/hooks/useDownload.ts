...
export function useDownload(region: string) {
    ...
    const downloadWithAutoFallback = async (id: string, settings: any, trackName?: string, artistName?: string, albumName?: string, playlistName?: string, position?: number, spotifyId?: string, durationMs?: number, releaseYear?: string, albumArtist?: string, releaseDate?: string, coverUrl?: string, spotifyTrackNumber?: number, spotifyDiscNumber?: number, spotifyTotalTracks?: number, spotifyTotalDiscs?: number, copyright?: string, publisher?: string) => {
        ...
        if (trackName && artistName) {
            try {
                ...
                if (existenceResults.length > 0 && existenceResults[0].exists) {
                    ...
                    return {
                        success: true,
                        message: t("fileAlreadyExistsMsg"),
                ...
                logger.warning(`File existence check failed: ${err}`);
            }
        }
        ...
        if (service === "auto") {
            ...
            for (const s of order) {
                if (s === "tidal") {
                    // Use pre-resolved URL when available; otherwise let the backend resolve via spotify_id.
                    const tidalServiceUrl = streamingURLs?.tidal_url || undefined;
                    try {
                        ...
                        const response = await downloadTrack({
                            ...
                            include_atmos: settings.includeAtmos ?? false,
                        });
                        if (response.success) {
                    ...
                    // Use pre-resolved URL when available; otherwise let the backend resolve via spotify_id.
                    const amazonServiceUrl = streamingURLs?.amazon_url || undefined;
                    try {
                    ...
                    const deezerServiceUrl = streamingURLs?.deezer_url || undefined;
                    try {
        ...
        const isDirectMode = settings.directMode === true && service !== "auto";
        if (isDirectMode) {
            logger.debug(`[direct] ${service}: ${trackName} - ${artistName}`);
        } else {
        ...
        const singleServiceResponse = await downloadTrack({
            ...
            use_single_genre: settings.useSingleGenre,
            embed_genre: settings.embedGenre,
            allow_fallback: isDirectMode ? false : true,
            include_atmos: service === "tidal" ? (settings.includeAtmos ?? false) : false,
        });
        if (!singleServiceResponse.success) {
            if (isDirectMode) {
                logger.error(`[direct] ${service} failed: ${singleServiceResponse.error || "unknown error"}`);
            }
            if (itemID) {
                const { MarkDownloadItemFailed } = await import("../../wailsjs/go/main/App");
        ...
        try {
            ...
            if (response.success) {
                if (response.already_exists) {
                    ...
                    if (response.quality_fallback) {
                        toast.warning(`${t("atmosFallback")} ${response.quality_fallback}`);
                    }
                }
    ...
    const handleDownloadSelected = async (selectedTracks: string[], allTracks: TrackMetadata[], folderName?: string, isAlbum?: boolean) => {
        if (selectedTracks.length === 0) {
            toast.error(t("noTracksSelected"));
            return;
        }
            ...
            try {
                ...
                if (response.success) {
                    if (response.already_exists) {
                        ...
                        if (response.quality_fallback) {
                            logger.warning(`atmos unavailable for ${track.name} — got ${response.quality_fallback}`);
                        }
                    }
                    if (response.file) {
    ...
    const handleDownloadAll = async (tracks: TrackMetadata[], folderName?: string, isAlbum?: boolean) => {
        ...
        if (tracksWithId.length === 0) {
            toast.error(t("noTracksForDownload"));
            return;
        }
    ...
    const handleStopDownload = () => {
        logger.info(t("logDownloadStopped"));
        shouldStopDownloadRef.current = true;
        toast.info(t("stoppingDownload"));
    };
